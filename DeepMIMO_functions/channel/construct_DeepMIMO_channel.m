...
function [channel, channel_LoS_status]=construct_DeepMIMO_channel(tx_ant_size, tx_rotation, tx_ant_spacing, rx_ant_size, rx_rotation, rx_ant_spacing, path_params, params)
    ...
    if ~params.activate_RX_filter
        path_const=sqrt(power/params.num_OFDM).*exp(1j*path_params.phase*ang_to_rad).*exp(-1j*2*pi*(k/params.num_OFDM)*delay_normalized);
        ...
        conv_vec = exp(1j*path_params.phase*ang_to_rad).*sqrt(power/params.num_OFDM) .* LP_fn; %Power of the paths and phase
